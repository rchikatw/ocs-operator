...
func (c *ocsConsumerManager) Create(ctx context.Context, onboard ifaces.StorageClientOnboarding) (string, error) {
	...
	klog.Info("Getting storage quota")
	...
	klog.Infof("Storage quota extracted %d", storageQuotaInGiB)

	consumerObj.Spec.StorageQuotaInGiB = int(storageQuotaInGiB)

	klog.Infof("Before Create Storage quota extracted %d", consumerObj.Spec.StorageQuotaInGiB)
	...
	klog.Infof("After Create Storage quota extracted %d", consumerObj.Spec.StorageQuotaInGiB)
...
func (c *ocsConsumerManager) EnableStorageConsumer(ctx context.Context, id string) error {
	...
	klog.Infof("Inside EnableStorageConsumer value of storage quota %v", consumerObj.Spec.StorageQuotaInGiB)
